...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (carModelMatch) {
      ...
      // Очистка: убрать "Рік випуску 1990", не трогать "6" и "2105"
      carModel = carModel
        .replace(/\s+\d+\s+Рік\s+випуску\s+\d{4}\b/i, '')  // пример: " 6 Рік випуску 1990"
        .replace(/\s+Рік\s+випуску\s+\d{4}\b/i, '')        // пример: " Рік випуску 1990"
