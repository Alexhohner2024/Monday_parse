...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!price && isGreenCard) {
      ...
      if (premiumIdx >= 0) {
        const afterPremium = fullText.substring(premiumIdx, premiumIdx + 3000);
        const priceMatch = afterPremium.match(/(\d[ \t\d]*\d)(?=[,.]00)/);
        if (priceMatch) {
          price = priceMatch[1].replace(/\s/g, '');
        } else {
          const fallbackMatch = afterPremium.match(/\n(\d[\d\s]*\d)\n/);
          if (fallbackMatch) {
            price = fallbackMatch[1].replace(/\s/g, '');
          }
        }
      }
    }
    ...
    // Формат: "3. СТРАХУВАЛЬНИК" або "3 СТРАХУВАЛЬНИК" з ПІБ в змішаному регістрі без слова "Найменування"
    if (!insuredName) {
      // Більш гнучкий варіант: працює і з точкою, і без точки після "3"
    ...
    if (!startDate && isGreenCard) {
      const gcStart1 = fullText.match(/Договоруз[ \t\n]*(\d{2}\.\d{2}\.\d{4})/i);
      const gcStart2 = fullText.match(/Договору\s+з\s+(\d{2}\.\d{2}\.\d{4})/i);
      const gcStart = gcStart1 || gcStart2;
      if (gcStart) {
        startDate = gcStart[1] + ', 00:00';
      }
    }
    ...
    // Green Card: "ДоговорупоDD.MM.YYYY" або "Договорупо DD.MM.YYYY" або "Договору по DD.MM.YYYY"
    if (!endDate && isGreenCard) {
      const gcEnd1 = fullText.match(/Договорупо[ \t\n]*(\d{2}\.\d{2}\.\d{4})/i);
      const gcEnd2 = fullText.match(/Договору\s+по\s+(\d{2}\.\d{2}\.\d{4})/i);
      const gcEnd = gcEnd1 || gcEnd2;
      if (gcEnd) {
        endDate = gcEnd[1];
      }
    }
